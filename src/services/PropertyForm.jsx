...
const PropertyForm = ({ show, onHide, formMode, currentProperty, onSave }) => {
  ...
  useEffect(() => {
    if (formMode === "edit" && currentProperty) {
      setFormData({
        ...currentProperty,
        images: currentProperty.images?.length > 0 ? [...currentProperty.images] : [""],
      });
    } else {
  ...
  const validateForm = () => {
    ...
    if (formData.beds && isNaN(formData.beds)) errors.beds = "Must be a valid number";
    if (formData.baths && isNaN(formData.baths)) errors.baths = "Must be a valid number";
    if (formData.sqft && isNaN(formData.sqft)) errors.sqft = "Must be a valid number";
    if (formData.priceNaira && isNaN(formData.priceNaira)) errors.priceNaira = "Must be a valid number";
  ...
  const handleSubmit = (e) => {
    ...
    const propertyData = {
      ...
      createdAt: formMode === "add" ? new Date().toISOString() : currentProperty.createdAt,
          ...
          <Row>
            ...
            <Col md={6}>
              <Form.Group className="mb-3" controlId="formBookings">
                ...
                />
                <Form.Text className="text-muted">
                  {formMode === "add" ? "Bookings will be tracked automatically" : "Number of client bookings for this property"}
                </Form.Text>
              </Form.Group>
            </Col>
          </Row>
